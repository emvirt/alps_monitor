 ...
 *  の責任を負わない．
 * 
 *  @(#) $Id: task_manage.c 552 2010-03-10 14:20:14Z ertl-honda $
 */
...
mig_tsk(ID tskid, ID prcid)
{
	...
	release_dual_tsk_lock_and_dispatch(f_p_pcb, t_p_pcb, dspreq);
	t_unlock_cpu();
...
act_tsk(ID tskid)
{
	...
	release_tsk_lock_and_dispatch(p_pcb, dspreq);
	t_unlock_cpu();
	...
	/* 異なるプロセッサに割り付けられているタスクならエラーとする */
		...
		if (!(TSTAT_WAITING(p_tcb->tstat) && TSTAT_WAIT_WOBJ(p_tcb->tstat))) {
			...
			if ((p_tcb->actprc == TPRC_NONE) || (p_tcb->actprc == p_pcb->prcid)) {
				...
				else if (TSTAT_WAITING(p_tcb->tstat) && !TSTAT_WAIT_WOBJ(p_tcb->tstat)) {
					/* オブジェクト待ち以外の待ち状態 */
					wait_dequeue_tmevtb(p_tcb);
				}
				else {
					/* 上記以外ならタスクの状態が変わっているためリトライ */
					release_dual_tsk_lock(p_pcb, t_p_pcb);
					goto retry;
				}
				make_dormant(p_tcb);
			...
			if ((p_tcb->actprc == TPRC_NONE) || (p_tcb->actprc == p_pcb->prcid)) {
				...
				/*
				 * 異なるオブジェクトに対する待ちになっていないかチェック
				 */
				if (p_wobjcb != p_tcb->p_wobjcb) {
					/*
					 *  異なるオブジェクトに対する待ちになっていればリトライ
					 */
				...
				/*
				 * 異なるオブジェクトに対する待ち，異なるプロセッサへのマ
				 * イグレーション，対象タスクがマイグレートしているならリ
				 * トライ．
				 */
				if ((p_wobjcb != p_tcb->p_wobjcb) |
					     (get_mp_p_pcb(p_tcb->actprc) != t_p_pcb) || (p_pcb != my_p_pcb)) {
					release_dual_tsk_lock(p_pcb, t_p_pcb);
					release_obj_lock(&GET_OBJLOCK(p_wobjcb));
					goto retry;
				}
...
chg_pri(ID tskid, PRI tskpri)
{
	...
	if (TSTAT_DORMANT(p_tcb->tstat)) {
		...
		else if (!TSTAT_WAIT_WOBJ(p_tcb->tstat)) {
			/* オブジェクト待ちでない場合 */
