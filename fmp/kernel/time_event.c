 ...
 *  の責任を負わない．
 * 
 *  @(#) $Id: time_event.c 479 2009-11-20 04:24:51Z ertl-honda $
 */
 ...
 *  遠い時刻）とみなして比較する．
 */
#define	EVTTIM_LT(p_tevtcb, t1, t2) (((t1) - p_tevtcb->current_time) < ((t2) - p_tevtcb->current_time))
#define	EVTTIM_LE(p_tevtcb, t1, t2) (((t1) - p_tevtcb->current_time) <= ((t2) - p_tevtcb->current_time))
...
signal_time(void)
{
	 ...
	 *  す．
	 */
	while ((my_p_tevtcb->last_index) > 0 && EVTTIM_LE(my_p_tevtcb, TMEVT_NODE(my_p_tevtcb, 1).time, my_p_tevtcb->next_time)) {
