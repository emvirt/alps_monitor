...
$   の責任を負わない．
$ 
$   @(#) $Id: kernel_check.tf 447 2009-10-16 08:51:47Z ertl-honda $
$  
$ =====================================================================
...
$	// 非タスクコンテキスト用のスタック領域の先頭番地のチェック
	$istkpt_table = SYMBOL("_kernel_istkpt_table")$
	$FOREACH prcid RANGE(1, TNUM_PRCID)$
		$istk = PEEK(istkpt_table, sizeof_STK_T_prt)$
		...
		$IF LENGTH(CHECK_STACK_NONNULL) && (istk == 0)$
			$ERROR ICE.TEXT_LINE[1]$E_PAR: 
				...
				"istk", ICS.ISTK[1], "DEF_ICS")$$END$
		$END$
		$istkpt_table = istkpt_table + sizeof_STK_T_prt$
	$END$
$END$
