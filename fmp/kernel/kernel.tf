...
$   の責任を負わない．
$ 
$   @(#) $Id: kernel.tf 574 2010-04-21 12:54:17Z ertl-honda $
$  
$ =====================================================================
...
$	// タスク初期化ブロック
	$IF EQ(TSK.STK[tskid], "NULL")$
		$TAB${ ($TSK.TSKATR[tskid]$), (intptr_t)($TSK.EXINF[tskid]$), ($TSK.TASK[tskid]$), INT_PRIORITY($TSK.ITSKPRI[tskid]$), ROUND_STK_T($TSK.STKSZ[tskid]$), (_kernel_stack_$tskid$), ($ALT(TSK.TEXATR[tskid],"TA_NULL")$), ($ALT(TSK.TEXRTN[tskid],"NULL")$), $CLASS_AFFINITY_INI[TSK.CLASS[tskid]]$, (uint_t)($CLASS_AFFINITY_MASK[TSK.CLASS[tskid]]$) }
	$ELSE$
		$TAB${ ($TSK.TSKATR[tskid]$), (intptr_t)($TSK.EXINF[tskid]$), ($TSK.TASK[tskid]$), INT_PRIORITY($TSK.ITSKPRI[tskid]$), ($TSK.STKSZ[tskid]$), ($TSK.STK[tskid]$), ($ALT(TSK.TEXATR[tskid],"TA_NULL")$), ($ALT(TSK.TEXRTN[tskid],"NULL")$), $CLASS_AFFINITY_INI[TSK.CLASS[tskid]]$, (uint_t)($CLASS_AFFINITY_MASK[TSK.CLASS[tskid]]$) }
	$END$
$END$$NL$
};$NL$
$NL$
...
$FOREACH order ISR.ORDER_LIST$
	$IF !LENGTH(ISR.CLASS[order])$
		$ERROR ISR.TEXT_LINE[order]$E_RSATR: $FORMAT(_("illegal class id of `%1%\' in %2%"), order, "ATT_ISR")$$END$
	$END$
$END$
...
$FOREACH inhno INH.ORDER_LIST$
	$IF ((inhno >> inhno_cpu_shift) & CLASS_AFFINITY_INI[INH.CLASS[inhno]]) == 0$
		$ERROR INH.TEXT_LINE[inhno]$E_PAR: $FORMAT(_("affinity processor of `0x%1$x\' in %2% is not match with class %3% initial processor."), +inhno, "DEF_INH", INH.CLASS[inhno])$$END$
	$END$
$END$

$ 
$ CFG_INTでは割込み番号を用いるため，割り付け可能プロセッサに含まれるかチェックする
$ 
$FOREACH intno INT.ORDER_LIST$
	$IF ((intno >> intno_cpu_shift) != 0) && (((1 << ((intno >> intno_cpu_shift) - 1)) & CLASS_AFFINITY_MASK[INT.CLASS[intno]]) == 0)$
		$ERROR INT.TEXT_LINE[intno]$E_PAR: $FORMAT(_("processor affinity mask of `%x1%\' in %2% is not match with class %3% processor affinity mask"), intno, "CFG_INT", INT.CLASS[intno])$$END$
	$END$
$END$

$ 
$ ATT_ISRでは割込み番号を用いるため，割り付け可能プロセッサに含まれるかチェックする
$ 
$FOREACH order ISR.ORDER_LIST$
	$IF ((ISR.INTNO[order] >> intno_cpu_shift) != 0) && (((1 << ((ISR.INTNO[order] >> intno_cpu_shift) - 1)) & CLASS_AFFINITY_MASK[ISR.CLASS[order]]) == 0)$
		$ERROR ISR.TEXT_LINE[order]$E_PAR: $FORMAT(_("processor affinity mask of `%1%\' in %2% is not match with class %3% processor affinity mask"), order, "ATT_ISR", ISR.CLASS[order])$$END$
	$END$
$END$
...
$	// inhatrが（［TA_NONKERNEL］）でない場合（E_RSATR）
	...
	$IF LENGTH(INTNO[inhno]) > 0$
		$intno = INTNO[inhno]$
		$IF LENGTH(FIND(INTNO_CFGINT_VALID, intno)) > 0$
$			// inhnoに対応するintnoに対するCFG_INTがない場合（E_OBJ）
			$IF !LENGTH(intno)$
				$ERROR INH.TEXT_LINE[inhno]$E_OBJ: $FORMAT(_("%1% `%2%\' corresponding to %3% `%4%\' is not configured with %5%"), "intno", intno, "inhno", inhno, "CFG_INT")$$END$
			$ELSE$
...
$		// intnoに対応するinhnoに対してDEF_INHがある場合（E_OBJ）
		$IF LENGTH(INH.INHNO[inhno])$
			$ERROR ISR.TEXT_LINE[order_for_error]$E_OBJ: $FORMAT(_("%1% `%2%\' in %3% is duplicated with %4% `%5%\'"), "intno", ISR.INTNO[order_for_error], "ATT_ISR", "inhno", inhno)$$END$
		$END$
...
$		// CFG_INTが所属するクラスが異なる場合（E_OBJ）
		$class = ISR.CLASS[order_for_error]$
		$IF class != INT.CLASS[intno] $
			$ERROR ISR.TEXT_LINE[order_for_error]$E_OBJ: $FORMAT(_("%1% and %2% for %3% `%4%\' is not defined in same class."), "ATT_ISR", "CFG_INT", "intno", ISR.INTNO[order_for_error])$$END$
		$END$

$		// ISRが全て同じクラスに属しているかのチェック
		$FOREACH order isr_order_list$
			$IF class != ISR.CLASS[isr_order_list]$
				$ERROR ISR.TEXT_LINE[order_for_error]$: $FORMAT(_("%1% `%2%\' is not defined in same class"), "intno", ISR.INTNO[order_for_error])$$END$
			$END$
		$END$
