 ...
 *  の責任を負わない．
 * 
 *  @(#) $Id: syslog.c 592 2010-05-07 01:39:37Z ertl-honda $
 */
...
typedef struct syslog_control_block{
	...
	uint_t	syslog_lowmask;		/* 低レベル出力すべき重要度 */    
}SYSLOGCB;
...
syslog_initialize(intptr_t exinf)
{
	...
	p_syslogcb->syslog_lowmask = LOG_UPTO(LOG_NOTICE);
}     
...
syslog_wri_log(uint_t prio, const SYSLOG *p_syslog)
{
	 ...
	 *  低レベル出力
	 */
	if ((p_syslogcb->syslog_lowmask & LOG_MASK(prio)) != 0U) {
...
syslog_msk_log(uint_t logmask, uint_t lowmask)
{
	...
	p_syslogcb->syslog_lowmask = lowmask;
...
syslog_ref_log(T_SYSLOG_RLOG *pk_rlog)
{
	...
	pk_rlog->lowmask = p_syslogcb->syslog_lowmask;
