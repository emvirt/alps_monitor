 ...
 *  の責任を負わない．
 * 
 *  @(#) $Id: prc_makeoffset.c 398 2009-09-15 15:32:17Z ertl-honda $
 */
...
makeoffset(void)
{
	...
	OFFSET_DEF2(PCB, target_pcb.except_nest_count, except_nest_count);
	OFFSET_DEF2(PCB, target_pcb.idf, idf);
	OFFSET_DEF2(PCB, target_pcb.iipm, iipm);
	OFFSET_DEF2(PCB, target_pcb.istkpt, istkpt);
	OFFSET_DEF2(PCB, target_pcb.p_iipm_mask_table, p_iipm_mask_table);
	OFFSET_DEF2(PCB, target_pcb.p_inh_table, p_inh_table);
	OFFSET_DEF2(PCB, target_pcb.p_inh_iipm_table, p_inh_iipm_table);
	OFFSET_DEF2(PCB, target_pcb.p_exch_table, p_exch_table);
	OFFSET_DEF2(PCB, target_pcb.prc_int_base, prc_int_base);
}
