...
extern void vic_init(uint64_t int_bit_pattern);
...
void timer_init(void (*callback)(void))
{
	...
	sil_wrw_mem((void *)(TARGET_TIMER_TCNTB), 1000*TO_CLOCK(TIC_NUME, TIC_DENO));
	...
	vic_init(INTNO_BITPAT(TIMER_FIQ2));
	
	__asm__ volatile ("msr cpsr, #(0x13)");
}
