...
#define VALID_PMQID(pmqid)	(TMIN_PMQID <= (pmqid) && (pmqid) <= tmax_pmqid)
#define VALID_SHMID(shmid)	(TMIN_SHMID <= (shmid) && (shmid) <= tmax_shmid)
...
#define CHECK_PMQID(pmqid) {								\
	if (!VALID_PMQID(pmqid)) {								\
		ercd = E_ID;										\
		goto error_exit;									\
	}														\
}

#define CHECK_SHMID(shmid) {								\
	if (!VALID_SHMID(shmid)) {								\
		ercd = E_ID;										\
		goto error_exit;									\
	}														\
}
