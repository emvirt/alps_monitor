...
#define TNUM_PMQID	$LENGTH(PMQ.ID_LIST)$$NL$
#define TNUM_SHMID	$LENGTH(SHM.ID_LIST)$$NL$
...
$FOREACH id PDQ.ID_LIST$
	#define $id$	$+id$$NL$
$END$
$FOREACH id PMQ.ID_LIST$
	#define $id$	$+id$$NL$
$END$
$FOREACH id SHM.ID_LIST$
	#define $id$	$+id$$NL$
$END$
...
$ 
/*$NL$
$SPC$*  Priority Message Queue Functions$NL$
$SPC$*/$NL$
$NL$

$ 優先度データキューID番号の最大値
const ID _kernel_tmax_pmqid = (TMIN_PMQID + TNUM_PMQID - 1);$NL$
$NL$

$IF LENGTH(PMQ.ID_LIST)$
	$FOREACH pmqid PMQ.ID_LIST$
$		// pmqatrが（［TA_TPRI］）でない場合（E_RSATR）
		$IF (PMQ.PMQATR[pmqid] & ~TA_TPRI) != 0$
			$ERROR PMQ.TEXT_LINE[pmqid]$E_RSATR: $FORMAT(_("illegal %1% `%2%\' of `%3%\' in %4%"), "pmqatr", PMQ.PMQATR[pmqid], pmqid, "CRE_PMQ")$$END$
		$END$

$		// (TMIN_MQPRI <= maxmpri && maxmpri <= TMAX_MQPRI)でない場合（E_PAR）
		$IF !(TMIN_MQPRI <= PMQ.MAXMPRI[pmqid] && PMQ.MAXMPRI[pmqid] <= TMAX_MQPRI)$
			$ERROR PMQ.TEXT_LINE[pmqid]$E_PAR: $FORMAT(_("illegal %1% `%2%\' of `%3%\' in %4%"), "maxmpri", PMQ.MAXMPRI[pmqid], pmqid, "CRE_PMQ")$$END$
		$END$

$		// pmqmbがNULLでない場合（E_NOSPT）
		$IF !EQ(PMQ.PMQMB[pmqid], "NULL")$
			$ERROR PMQ.TEXT_LINE[pmqid]$E_NOSPT: $FORMAT(_("illegal %1% `%2%\' of `%3%\' in %4%"), "pmqmb", PMQ.PMQMB[pmqid], pmqid, "CRE_PMQ")$$END$
		$END$

$		// message blocks region
		$IF PMQ.PMQCNT[pmqid]$
			static char_t _kernel_pmqbuffer_$pmqid$[$PMQ.PMQCNT[pmqid]$][$PMQ.MAXMSGSZ[pmqid]$];$NL$
			static PMQMB _kernel_pmqmb_$pmqid$[$PMQ.PMQCNT[pmqid]$];$NL$
		$END$
	$END$

$	// 優先度データキュー初期化ブロックの生成
	const PMQINIB _kernel_pmqinib_table[TNUM_PMQID] = {$NL$
	$JOINEACH pmqid PMQ.ID_LIST ",\n"$
		$TAB${ ($PMQ.PMQATR[pmqid]$), ($PMQ.PMQCNT[pmqid]$), ($PMQ.MAXMSGSZ[pmqid]$), ($PMQ.MAXMPRI[pmqid]$), $IF PMQ.PMQCNT[pmqid]$(_kernel_pmqmb_$pmqid$)$ELSE$NULL$END$, $IF PMQ.PMQCNT[pmqid]$(&_kernel_pmqbuffer_$pmqid$[0][0])$ELSE$NULL$END$ }
	$END$$NL$
	};$NL$
	$NL$

$	// 優先度データキューコントロールブロック
	PMQCB _kernel_pmqcb_table[TNUM_PMQID];$NL$
$ELSE$
	TOPPERS_EMPTY_LABEL(const PMQINIB, _kernel_pmqinib_table);$NL$
	TOPPERS_EMPTY_LABEL(PMQCB, _kernel_pmqcb_table);$NL$
$END$
$NL$
...
$ 
/*$NL$
$SPC$*  Shared Memory Functions$NL$
$SPC$*/$NL$
$NL$

$ Maximum shared memory ID
const ID _kernel_tmax_shmid = (TMIN_SHMID + TNUM_SHMID - 1);$NL$
$NL$

$IF LENGTH(SHM.ID_LIST)$
	$FOREACH shmid SHM.ID_LIST$
$		// if shmadd is NULL we create a local memory region
		$IF EQ(SHM.SHMADD[shmid], "NULL")$
			static char _kernel_shmmb_$shmid$[$SHM.SHMSZ[shmid]$];$NL$
		$END$
	$END$

$	// create the initialization table
	const SHMINIB _kernel_shminib_table[TNUM_SHMID] = {$NL$
	$JOINEACH shmid SHM.ID_LIST ",\n"$
		$TAB${ ($SHM.SHMATR[shmid]$), $IF EQ(SHM.SHMADD[shmid], "NULL")$(intptr_t)(_kernel_shmmb_$shmid$)$ELSE$($SHM.SHMADD[shmid]$)$END$, ($SHM.SHMSZ[shmid]$) }
	$END$$NL$
	};$NL$
	$NL$

$	// Shared memory control block table
	SHMCB _kernel_shmcb_table[TNUM_SHMID];$NL$
$ELSE$
	TOPPERS_EMPTY_LABEL(const SHMINIB, _kernel_shminib_table);$NL$
	TOPPERS_EMPTY_LABEL(SHMCB, _kernel_shmcb_table);$NL$
$END$
$NL$
...
$IF LENGTH(PMQ.ID_LIST)$	_kernel_initialize_primsgq();$NL$$END$
$IF LENGTH(SHM.ID_LIST)$	_kernel_initialize_shmem();$NL$$END$
