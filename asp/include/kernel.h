...
typedef struct t_rpmq {
	ID		stskid;		/* priority msg queue waiting sending task id */
	ID		rtskid;		/* priority msg queue waiting receive task id */
	uint_t	spmqcnt;	/* priority msg queue current number of msg */
} T_RPMQ;
...
extern ER		snd_pmq(ID pmqid, intptr_t data, uint_t msgsz, PRI datapri) throw();
extern ER		psnd_pmq(ID pmqid, intptr_t data, uint_t msgsz, PRI datapri) throw();
extern ER		ipsnd_pmq(ID pmqid, intptr_t data, uint_t msgsz, PRI datapri) throw();
extern ER		tsnd_pmq(ID pmqid, intptr_t data, uint_t msgsz,
										PRI datapri, TMO tmout) throw();
extern ER		rcv_pmq(ID pmqid, intptr_t buffer, uint_t *p_msgsz, PRI *p_datapri) throw();
extern ER		prcv_pmq(ID pmqid, intptr_t buffer, uint_t *p_msgsz, PRI *p_datapri) throw();
extern ER		trcv_pmq(ID pmqid, intptr_t buffer, uint_t *p_msgsz,
										PRI *p_datapri, TMO tmout) throw();
extern ER		ini_pmq(ID pmqid) throw();
extern ER		ref_pmq(ID pmqid, T_RPMQ *pk_rpmq) throw();

extern ER		get_shm(ID shmid, intptr_t *p_shmadd, uint_t *p_shmsz) throw();
...
#define TTW_SPMQ		UINT_C(0x0400)	/* priority message queue send wait */
#define TTW_RPMQ		UINT_C(0x0800)	/* priority message queue recv wait */
...
#define TMIN_MQPRI		1			/* minimum message priority */
#define TMAX_MQPRI		16			/* maximum message priority */
