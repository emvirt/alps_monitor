...
#define S3C64XX_GPM_BASE        (S3C64XX_PA_GPIO + 0x0820)
...
void task(intptr_t exinf)
{
	...
	*((volatile uint32_t *)S3C64XX_GPMCON) = 0x1111;

	if (p++ % 2){
		*((volatile uint32_t *)S3C64XX_GPMDAT) &= ~0xF;
// 		syslog(LOG_NOTICE, "*(0x%X)=0x%X(should be 0)", S3C64XX_GPMDAT, *((uint32_t *)S3C64XX_GPMDAT));
	}else{
		*((volatile uint32_t *)S3C64XX_GPMDAT) |= 0xF;
// 		syslog(LOG_NOTICE, "*(0x%X)=0x%X(should be F)", S3C64XX_GPMDAT, *((uint32_t *)S3C64XX_GPMDAT));
	}
}
...
void main_task(intptr_t exinf)
{
	...
	*((uint32_t *)S3C64XX_GPMCON) = 0x1111;
// 	syslog(LOG_NOTICE, "*(0x%X)=0x%X", S3C64XX_GPMCON, *((uint32_t *)S3C64XX_GPMCON));
