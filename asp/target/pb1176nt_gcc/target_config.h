 ...
 *  割込み優先度マスクは0x0 〜 0xf の16段階で値が大きいほど低優先度である．
 ...
 */
#ifndef TOPPERS_MACRO_ONLY
#define EXT_IPM(iipm)    ((PRI)(iipm - 0x0fU))    /* 内部表現を外部表現に */
#define INT_IPM(ipm)    ((uint8_t)(ipm + 0x0fU))/* 外部表現を内部表現に */
#else /* TOPPERS_MACRO_ONLY */
#define EXT_IPM(iipm)    (iipm - 0xf)            /* 内部表現を外部表現に */
#define INT_IPM(ipm)    (ipm + 0xf)                /* 外部表現を内部表現に */
#endif /* TOPPERS_MACRO_ONLY */
 ...
 *  割込み要求ラインの属性の設定
 *
 *  PICは，全てレベルトリガのため，属性で，トリガの指定はでき
 *  ない  
 */
